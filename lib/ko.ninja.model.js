...
define([
	...
	/**
	 * This is the core class upon which all other Model instances are based.
	 *
	 * @class Model
	 */
	var Model = function() {

		if (!Model.esperanto_initialized) {
			Model.initializeEsperanto();
		}
	...
	Model.esperanto_initialized = false;

	/**
	 * Initializes the default data store for all models.
	 */
	Model.initializeEsperanto = function() {
		switch (Model.storage.method) {
			case 'localStorage':
				Model.esperanto = new Esperanto('localStorage');
				break;
		}
	};

	_.extend(Model.prototype, /** @lends Model.prototype */ {
		...
		*/
		...
		'fetch': function() {
			...
			if (this.isNew()) {
				...
				if (this.storage) {} else {
					...
					Model.esperanto.findWhere(this.model, props).done(function(result) {
		...
		'create': function() {
			...
			if (this.validationErrorCount() > 0) {
				...
				if (this.storage) {} else {
					Model.esperanto.create(this.model, data).done(function(result) {
		...
		'update': function() {
			...
			if (this.validationErrorCount() > 0) {
				...
				if (!this.id()) {
					...
					delete data[this.idAttribute];
					if (this.storage) {} else {
						Model.esperanto.save(this.model, this.id(), data).done(function(result) {
							self.setID(result[self.idAttribute]);
							d.resolve();
						}).fail(function(err) {
							d.reject('Data layer failed to save.');
						});
					}
				}
			}
		...
		'destroy': function() {
			...
			if (this.isNew()) {
				...
				var id = this.id();
				Model.esperanto.destroy(this.model, id).done(function(result) {
